...
const Header: React.FC<HeaderProps> = ({ onServicesClick, onPortfolioClick, onPricingClick, onFaqClick, onContactClick }) => {
  ...
  const MobileNavLink: React.FC<{onClick: () => void, children: React.ReactNode}> = ({ onClick, children }) => (
    <a onClick={() => handleMobileLinkClick(onClick)} className="relative group text-4xl font-bold text-white hover:text-sky-400 transition-colors pb-2">
      <span className="relative z-10">{children}</span>
      <span className="absolute bottom-0 left-0 w-full h-[2px] bg-sky-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out origin-center"></span>
    </a>
  );
  ...
  return (
    <header className={`fixed w-full top-0 z-40 md:py-2 transition-all duration-300 border-b border-sky-500 bg-slate-900/80 ${isScrolled ? 'backdrop-blur-lg shadow-[0_4px_30px_rgba(99,102,241,0.2)]' : ''}`}>
      <nav className="container mx-auto px-6 h-[4.5rem] flex justify-between items-center">
        <div className="flex text-2xl font-bold  items-center space-x-2 cursor-pointer text-white" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <a onClick={onContactClick} className="cursor-pointer bg-sky-500 hover:bg-sky-600 text-white font-bold py-2 px-4 rounded-lg transition-all duration-300 transform hover:scale-105">
            Contact Us
          </a>
        </div>
      ...
      <div className={`
        ...
        ${isMobileMenuOpen ? 'max-h-[100vh] border-t border-slate-700/50' : 'max-h-0'}
      `}>
        <nav className="flex flex-col items-center justify-center space-y-10 w-full" style={{ height: 'calc(100vh - 4.5rem)' }}>
